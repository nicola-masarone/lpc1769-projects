...
The purpose of this function is to change the count value of the SysTickTimer and therefore the on / off speed of the LEDs. The static int counter variable (initialized to 0) can take only three values: 0, 1 and 2, then it returns to the value 0. Subsequently, the System Tick Timer is temporarily stopped to modify its count value. With the switch selector you choose (based on the current counter value) which value to assign to the System Tick Timer, passing from the maximum value (minimum speed), to the average value (average speed) to the minimum value (maximum speed). The last instructions are used to reactivate the System Tick Timer (with the count value just changed) and to cancel the button press event because otherwise it would take us back to this function indefinitely even if we stop pressing the button.
