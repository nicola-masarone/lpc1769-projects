...
{
	static int PWM_dc=PWM_dc_MIN;

	//ciclo di rotazione da posizione minima a posizione massima
...
int main(void) {
    ...
    STRELOAD = SYSTICK_MAX_VAL/8;	// Carica il contatore per il systick timer
	...
	PWM1MR2 = PWM1MR0/PWM_period*PWM_dc_MIN;	// prepara per MR2 un duty cycle min (posizione inizio rotazione)
